...
router.put('/', function (req, res, next) {
  ...
  db.run("UPDATE user SET first_name = ?, last_name = ?, email = ?, password = ?, team = ? WHERE id = ? ",
    user.f_name, user.l_name, user.email, user.password, user.team, user_id, function (err) {

      if (err) {
        console.log(err);
        console.log('User info update failed!');
        res.status(202).send({success: false, msg: 'User info update failed!'});
      }
      else {
        console.log('Successful user info update!');
        //res.json({success: true, msg: 'Successful user info update!'});
      }
      //res.end();
    });
